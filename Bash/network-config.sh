...
LAN=$(ip a |awk '/: ens34/{gsub(/:/,"");print $2}')
LANA=$(ip a s $LAN |awk '/inet /{gsub(/\/.*/,"");print $2}')
LANH=$(getent hosts $LANA | awk '{print $2}')
EXIT=$(curl -s icanhazip.com)
EXIN=$(getent hosts $EXIT | awk '{print $2}')
ROUTERA=$(ip route show | grep -i 'default via'| awk '{print $3 }')
